...
def run_persistent_mode(args):
    ...
    """
    # Print startup message to stderr to avoid interfering with stdout communication
    print("🚀 Starting Zonos worker in persistent mode...", file=sys.stderr)
    sys.stderr.flush()
    ...
    try:
        ...
        print("🛑 Persistent worker shutting down...", file=sys.stderr)
    except Exception as e:
...
def process_request(request, model_cache):
    ...
    try:
        ...
        if device_arg == 'mps' or (device_arg == 'auto' and hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()):
            print(f"🍎 Disabling torch.compile for MPS compatibility", file=sys.stderr)
            os.environ['TORCH_COMPILE_DISABLE'] = '1'
            os.environ['TORCHDYNAMO_DISABLE'] = '1'
            # Disable Metal API validation to prevent crashes
            os.environ['METAL_DEVICE_WRAPPER_TYPE'] = ''
            print(f"🍎 Disabled Metal API validation to prevent buffer validation crashes", file=sys.stderr)
        ...
        print(f"🎤 Processing Zonos request:", file=sys.stderr)
        print(f"   • Model: {model_name}", file=sys.stderr)
        print(f"   • Reference Audio: {ref_audio}", file=sys.stderr)
        print(f"   • Reference Text: {ref_text[:100]}{'...' if len(ref_text) > 100 else ''}", file=sys.stderr)
        print(f"   • Generation Text: {gen_text[:100]}{'...' if len(gen_text) > 100 else ''}", file=sys.stderr)
        ...
        if model_key in model_cache:
            print(f"✓ Using cached Zonos model: {model_name}", file=sys.stderr)
            model = model_cache[model_key]
            load_time = 0  # No loading time for cached model
        else:
            print(f"📥 Loading Zonos model: {model_name}", file=sys.stderr)
            ...
            if str(device) == 'mps':
                print(f"🍎 Disabling torch.compile for MPS device", file=sys.stderr)
                os.environ['TORCH_COMPILE_DISABLE'] = '1'
            ...
            print(f"✓ Model loaded and cached in {load_time:.2f}s", file=sys.stderr)
    ...
    if not espeak_path:
        ...
        for path in common_paths:
            if os.path.exists(path):
                ...
                print(f"✓ Found espeak-ng at: {path}", file=sys.stderr)
                break
        else:
            raise RuntimeError("espeak-ng not found. Please install it with: brew install espeak-ng")
    else:
        print(f"✓ espeak-ng found in PATH: {espeak_path}", file=sys.stderr)
...
def get_device(device_arg):
    ...
    if str(device) == 'mps' and os.environ.get('METAL_DEVICE_WRAPPER_TYPE') == 'MTLDebugDevice':
        print(f"⚠️ Metal API Validation detected, forcing CPU mode to prevent crashes", file=sys.stderr)
        device = torch.device("cpu")
...
def generate_audio_with_model(model, device, ref_audio, ref_text, gen_text, output_file, config):
    ...
    if str(device) != 'cpu':
        wav = wav.to(device)
        print(f"🎯 Audio moved to device: {device}", file=sys.stderr)
    
    # Create speaker embedding
    print("🎭 Creating speaker embedding...", file=sys.stderr)
    try:
        speaker = model.make_speaker_embedding(wav, sampling_rate)
        if str(device) != 'cpu':
            speaker = speaker.to(device)
            print(f"🎯 Speaker embedding on device: {device}", file=sys.stderr)
    except RuntimeError as e:
        if "mps" in str(e).lower() or "cpu" in str(e).lower() or "device" in str(e).lower():
            print(f"⚠️ Device issue with speaker embedding, falling back to CPU: {e}", file=sys.stderr)
            wav_cpu = wav.cpu()
            ...
            if str(device) != 'cpu':
                speaker = speaker.to(device)
                print(f"🎯 Speaker embedding moved back to device: {device}", file=sys.stderr)
        else:
            raise e
    
    # Prepare conditioning
    print("🎯 Preparing conditioning...", file=sys.stderr)
    ...
    print(f"🌍 Language mapping: {raw_language} -> {zonos_language}", file=sys.stderr)
    ...
    if 'zonos_settings' in config:
        ...
        print(f"🎭 Emotion vector: {emotion_vector}", file=sys.stderr)
    
    if emotion_vector is None:
        emotion_vector = [0.3077, 0.0256, 0.0256, 0.0256, 0.0256, 0.0256, 0.2564, 0.3077]
        print(f"🎭 Using default emotion vector: {emotion_vector}", file=sys.stderr)
    ...
    print(f"🔧 Final parameters: fmax={fmax}, pitch_std={pitch_std}, speaking_rate={speaking_rate}", file=sys.stderr)
    
    # Create conditioning dictionary
    print(f"🎯 Creating conditioning dictionary...", file=sys.stderr)
    ...
    if 'seed' in config:
        ...
        print(f"🎲 Set seed: {config['seed']}", file=sys.stderr)
    elif 'zonos_settings' in config and 'seed' in config['zonos_settings']:
        torch.manual_seed(config['zonos_settings']['seed'])
        print(f"🎲 Set seed from zonos_settings: {config['zonos_settings']['seed']}", file=sys.stderr)
    
    # Move tensors to device
    print(f"🔍 Checking cond_dict tensor devices:", file=sys.stderr)
    for key, value in cond_dict.items():
        if isinstance(value, torch.Tensor):
            print(f"   • {key}: {value.device}", file=sys.stderr)
            if str(value.device) != str(device):
                print(f"   • Moving {key} from {value.device} to {device}", file=sys.stderr)
                cond_dict[key] = value.to(device)
    
    # Prepare conditioning
    print(f"🎯 Preparing conditioning with model...", file=sys.stderr)
    try:
        conditioning = model.prepare_conditioning(cond_dict)
    except RuntimeError as e:
        if "cpu" in str(e).lower() and "mps" in str(e).lower():
            print(f"⚠️ Device mismatch in prepare_conditioning, trying with CPU tensors: {e}", file=sys.stderr)
            cond_dict_cpu = {}
            for key, value in cond_dict.items():
                if isinstance(value, torch.Tensor):
            ...
            print(f"✓ Successfully prepared conditioning using CPU workaround", file=sys.stderr)
        else:
            raise e
    
    # Ensure conditioning is on correct device
    print(f"🔍 Checking conditioning tensor devices after preparation:", file=sys.stderr)
    if isinstance(conditioning, torch.Tensor):
        print(f"   • conditioning tensor: {conditioning.device}", file=sys.stderr)
        if str(conditioning.device) != str(device):
            print(f"   • Moving conditioning from {conditioning.device} to {device}", file=sys.stderr)
            conditioning = conditioning.to(device)
    elif isinstance(conditioning, dict):
        for key, value in conditioning.items():
            if isinstance(value, torch.Tensor):
    ...
    print(f"✓ All conditioning tensors confirmed on {device}", file=sys.stderr)
    ...
    try:
        ...
        if "mps" in str(e).lower() and "cpu" in str(e).lower():
            print(f"⚠️ MPS device issue, falling back to CPU: {e}", file=sys.stderr)
            model = model.to('cpu')
            ...
            with torch.inference_mode():
                codes = model.generate(conditioning)
                wavs = model.autoencoder.decode(codes).cpu()
            print("✓ Successfully generated audio using CPU fallback", file=sys.stderr)
        else:
            raise e
    except Exception as e:
        if "Metal API Validation" in str(e) or "validateComputeFunctionArguments" in str(e):
            print(f"⚠️ Metal API validation error, falling back to CPU: {e}", file=sys.stderr)
            model = model.to('cpu')
            ...
            with torch.inference_mode():
                codes = model.generate(conditioning)
                wavs = model.autoencoder.decode(codes).cpu()
            print("✓ Successfully generated audio using CPU fallback for Metal API issue", file=sys.stderr)
        else:
            raise e
    
    generation_time = time.perf_counter() - generation_start
    print(f"✓ Audio generated in {generation_time:.2f}s", file=sys.stderr)
    ...
    print(f"✓ Zonos generation completed successfully", file=sys.stderr)
